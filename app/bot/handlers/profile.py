...
def _profile_text(snapshot: ProfileSnapshot) -> str:
    ...
    return (
        ...
        ".━━━━━━━━━━━━━━━━━━━━━━━.\n"
        "🏰 قلمرو و مدرسه\n\n"
        f"🏯 سطح دژ: {castle_level}\n"
        f"❤️ استحکام دژ: {castle_strength}\n"
        f"🛡 قدرت دفاع: {defense_power}\n"
        f"👨‍🏫 دبیرها: {snapshot.active_teachers_count} فعال از "
        f"{snapshot.teachers_count}\n\n"
        ".━━━━━━━━━━━━━━━━━━━━━━━.\n"
        "⚔️ کارنامه نبرد\n\n"
        f"⚔️ حمله‌های انجام‌شده: {snapshot.attacks_sent}\n"
        f"🏆 حمله‌های موفق: {snapshot.successful_attacks}\n"
        f"🎯 حمله‌های در انتظار: {snapshot.pending_attacks}\n"
        f"🎖 حمله‌های دریافتی: {snapshot.attacks_received}\n"
