...
class DailyQuestRepository:
    async def list_for_date(
        self, session: AsyncSession, activity_date: date, *, active_only: bool = True
    ):
    ...
    async def get(
        self, session: AsyncSession, quest_id: int, *, for_update: bool = False
    ):
    ...
    async def progress(
        ...
        *,
        ...
        query = select(DailyQuestProgress).where(
