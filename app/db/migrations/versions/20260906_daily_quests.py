...
def upgrade() -> None:
    ...
    op.create_index(
        "ix_daily_quests_date_active", "daily_quests", ["activity_date", "is_active"]
    )
    op.create_table(
        ...
        sa.Column(
            "quest_id",
            sa.BigInteger(),
            sa.ForeignKey("daily_quests.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column("activity_date", sa.Date(), nullable=False),
        ...
        sa.CheckConstraint(
            "progress >= 0", name="ck_daily_quest_progress_non_negative"
        ),
    )
    op.create_index(
        "ix_daily_quest_progress_user_date",
        "daily_quest_progress",
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_daily_quest_progress_user_date", table_name="daily_quest_progress"
    )
